...
#include "llvm/Transforms/IPO/InputGenerationTypes.h"
...
struct InputGenRTTy {
  InputGenRTTy(const char *ExecPath, const char *OutputDir,
    ...
    DefaultIntDistrib = std::uniform_int_distribution<>(0, 256);
...
int main(int argc, char **argv) {
  ...
  std::atexit([]() {
    if (InputGenRT) {
      ...
      std::cout << "dump time: " << (end_run - start_run).count() << std::endl;
      delete InputGenRT;
      InputGenRT = nullptr;
    }
    if (DynLibHandle)
  ...
  RunInputGen = [&]() {
    ...
    std::cout << "init time: " << (end_init - start_init).count() << std::endl;
    auto start_run = std::chrono::high_resolution_clock::now();
    EntryFn(argc, argv);
    auto end_run = std::chrono::high_resolution_clock::now();
    std::cout << "run time: " << (end_run - start_run).count() << std::endl;
    exit(0);
  };
